...
note_ornamentation_map = {  # for notations/, modified from EasyABC
    ...
    "fermata": "!fermata!",
    "arpeggiate": "!arpeggio!",
    "articulations/tenuto": "!tenuto!",  # not sure whether this is the right translation
...
class Parser:
    ...
    def doNotations(s, note, nttn, isTab):
        ...
        if snaar is not None and isTab:
            if s.tstep:
    ...
    def doNote(s, n):  # parse a musicXML note tag
        ...
        if (
            e is not None
            and e.text == "none"
            and (not isTab or v in s.hasStems or s.tstep)
        ):
        ...
        if n.find("unpitched") is not None:
            ...
            instId = instr.get("id") if instr is not None else "dummyId"
    ...
    def doDirection(s, e, i, es):  # parse a musicXML direction tag
        def addDirection(x, vs, tijd, stfnum):
            ...
            for v in vs:
                if tijd is not None:  # insert at time of encounter
                    s.msc.appendElemT(
